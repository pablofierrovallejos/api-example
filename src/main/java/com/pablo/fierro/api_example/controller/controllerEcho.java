...
import com.pablo.fierro.api_example.generator.MultipartImage;
...
public class controllerEcho {
    ...
    @GetMapping("/qr/{sTexto}")
    MultipartImage qr(@PathVariable("sTexto") String sTexto) throws OutputException, BarcodeException, IOException {
        BufferedImage bufferedImage = qrGen.generateQrCode(sTexto, 350, 350);
    ...
    MultipartFile pdf417(@PathVariable("sTexto") String sTexto) throws BarcodeException, OutputException, IOException {
        final Barcode barcode = BarcodeFactory.createPDF417(sTexto);
